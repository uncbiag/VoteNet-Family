...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        # if sample['name'] == 's1':
        # print("Name {}, Coor {}".format(sample['name'], sample['coordinate']))
...
class BalancedRandomCrop(object):
    ...
    def __init__(self, output_size, threshold=0.01, random_state=None):
        ...
        self.current_class = 56  # tag that which class should be focused on currently
    ...
    def __call__(self, sample):
        ...
        if self.current_class == 0:  # random crop a patch for background
            ...
            while not contain_label:
                ...
                if float(np.sum(seg_crop_np == self.current_class)) / float(seg_crop_np.size) > self.threshold:  # judge if the patch satisfy condition
                    contain_label = True
                i = i + 1
                # print(sample['name'], i, self.current_class, float(np.sum(seg_crop_np == self.current_class)), float(seg_crop_np.size), float(np.sum(seg_crop_np == self.current_class)) / float(seg_crop_np.size), self.threshold)
                # if (i > 1000):
                #     ValueError('Warning! not cropping!')
        ...
        if self.current_class > 56:
            self.current_class = 0

        # if sample['name'] == 's1':
        #     print("Name {}, Class {}, Coor {}".format(sample['name'], sample['class'], sample['coordinate']))
        return sample
...
#     my_balanced_random_crop = MyBalancedRandomCrop(patch_size, threshold=sample_threshold.tolist(), label_list=label_list, max_crop_num=max_crop_num)
#
...
#                 seg_crop_np = cropping(seg_np.astype(np.float32), start_coord, size_new)
...
#             seg_crop = seg_np[start_coord[0]:after_coord[0], start_coord[1]:after_coord[1], start_coord[2]:after_coord[2]].copy()
