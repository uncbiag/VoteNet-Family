...
def prior_reshape(priors, n_classes):
    variable = False
    if isinstance(priors, Variable):
        priors = priors.data
        variable = True
    output = torch.zeros((priors.shape[0], n_classes) + priors.shape[1:])
    for i in range(n_classes):
        output[:,i,...] += 6.22*(priors == i).type('torch.FloatTensor')
...
def train_model(model, training_data_loader, validation_data, optimizer, args, config, scheduler=None):
    ...
    while current_epoch <= config['n_epochs']:
        ...
        for i, (images, segs, priors, names) in enumerate(training_data_loader):
            ...
            segs = Variable(segs.float())
            # segs = Variable(segs)
            priors = Variable(priors)

            imgs_and_priors = torch.cat((images, priors), 1)

            optimizer.zero_grad()

            output = model(imgs_and_priors.cuda())

            output_flat = output.permute(0, 2, 3, 4, 1).contiguous().view(-1, config['n_classes'])
            segs_flat = segs.view(segs.numel())

            if config['loss'] == 'dice':
                loss = bio_loss.mydice_loss(output_flat, segs_flat.cuda())
            elif config['loss'] == 'dice2':
                loss = bio_loss.mydice_loss2(output_flat, segs_flat.cuda())
            elif config['loss'] == 'binary_cross_entropy':
                output_flat_1 = output.permute(0, 2, 3, 4, 1).contiguous().view(config['batch_size'], -1)
                segs_flat_1 = segs_flat.view(config['batch_size'], -1).float()
                # pos_weight = (1.0 - segs_flat) + 0.2
                # pos_weight = segs_flat*5
                loss = F.binary_cross_entropy_with_logits(output_flat_1, segs_flat_1.cuda())#, pos_weight=pos_weight.cuda())
            ...
            if i % config['print_batch_period'] == config['print_batch_period'] - 1:  # print every 10 mini-batches
                ...
                print('Epoch: {:0d} [{}/{} ({:.0f}%)] loss: {:.3f} lr:{} ({:.3f} sec/batch) {}'.format
                      (current_epoch, (i + 1) * config['batch_size'], iters_per_epoch,
                       float((i + 1) * config['batch_size']) / iters_per_epoch * 100,
        ...
        if current_epoch % config['valid_epoch_period'] == 0:
            ...
            for j in range(5):
                ...
                image_tiles, segs, priors, names = validation_data[j]
                if config['if_weight_loss']:
                    if config['weight_mode'] == 'dynamic':
                        loss_weight = weight_from_truth(segs, config['n_classes'])
                    elif config['weight_mode'] == 'const':
                ...
                for i in range(0, np.ceil(float(image_tiles.size()[0]) / float(valid_batch_size)).astype(int)):
                    ...
                    prior_input = priors.narrow(0, valid_batch_size * i,
                                                    valid_batch_size if valid_batch_size * (i + 1) <= priors.size()[0]
                                                    else priors.size()[0] - valid_batch_size * i)
                    with torch.no_grad():
                        temp_input = Variable(temp_input)
                        prior_input = Variable(prior_input)

                    # predict through model 1
                    cat_input = torch.cat((temp_input, prior_input), 1)
                    with torch.no_grad():
                        cat_input = Variable(cat_input)
                    temp_output = model(cat_input.cuda()).cpu().data
                ...
                predictions = (F.sigmoid(predictions) > 0.5).squeeze()
                # pred_assemble = validation_data.transform.transforms[0].assemble(torch.max(predictions, 1)[1], if_itk=False)
                pred_assemble = validation_data.transform.transforms[0].assemble(predictions, if_itk=False)
            ...
            save_criter = np.max(dice_all)*1000+np.min(dice_all)

            if config['lr_mode'] == 'plateau':
                scheduler.step(dice_avg, epoch=current_epoch)

            is_best = False
            if save_criter > best_score:
                is_best = True
                best_score = save_criter
                print('Best score: {}'.format(dice_avg))
            ...
            print('Epoch: {:0d} Validation: Dice Avg: {:.3f} Dice_lowest: {:.3f} Dice_highest:{:.3f} ({:.3f} sec) {}'.format
                  (current_epoch, dice_avg, np.min(dice_all),
                   np.max(dice_all), time.time() - start_time,
                   datetime.datetime.now().strftime("%D %H:%M:%S")))
